...
describe("HorseCard", () => {
  it("renders the horse name", () => {
    ...
    expect(wrapper.get('[data-testid="horse-name"]').text()).toBe(
      "Thunder Storm",
    );
  });
  ...
  it("renders the condition label with the correct value", () => {
    const wrapper = mount(HorseCard, { props: { horse } });
    expect(wrapper.get('[data-testid="condition-label"]').text()).toContain(
      "75",
    );
  });
  ...
  it("renders the color swatch with the horse color", () => {
    ...
    // jsdom normalises hex colours to rgb(); check the element's inline style directly
