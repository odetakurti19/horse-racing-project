...
const handleGenerate = async (): Promise<void> => {
  ...
  await store.dispatch("race/generateSchedule", store.state.horse.horses)
}
...
<template>
  <div
    class="flex items-center gap-6 px-4 py-3 flex-wrap border-b border-(--border-color) bg-(--panel-bg) rounded-lg"
    data-testid="control-panel"
  >
    <span class="text-[1.2rem] font-bold text-(--accent-gold) whitespace-nowrap">
      🏇 Horse Racing
    </span>
    ...
    <div class="flex gap-3 flex-wrap">
      ...
      <Button
        v-if="store.state.race.status === RACE_STATUS.RUNNING"
        label="Pause"
        icon="pi pi-pause"
        severity="warn"
      ...
      <Button
        v-if="store.state.race.status === RACE_STATUS.PAUSED"
        label="Resume"
        icon="pi pi-play"
        severity="success"
    ...
    <div class="ml-auto capitalize" data-testid="status-badge">
      <Tag :value="store.state.race.status" :severity="statusSeverity[store.state.race.status]" />
    </div>
  </div>
</template>
