...
describe("createRound", () => {
  it("creates a round with idle status and empty arrays", () => {
    ...
    expect(round.status).toBe(RACE_STATUS.IDLE);
  });
});

describe("createRaceSchedule", () => {
  it(`creates a schedule with ${TOTAL_ROUNDS} rounds`, () => {
    const schedule = createRaceSchedule();
    expect(schedule.rounds).toHaveLength(TOTAL_ROUNDS);
  });
...
describe("computeFinishTime", () => {
  ...
  it("a higher-condition horse finishes faster (with fixed randomness)", () => {
    ...
    expect(computeFinishTime(fast, 1200)).toBeLessThan(
      computeFinishTime(slow, 1200),
    );
    vi.restoreAllMocks();
  });
  ...
  it("finish time increases with distance", () => {
    vi.spyOn(Math, "random").mockReturnValue(0.5);
    const horse = createHorse(1, "Test", "#FF0000", 80);
    expect(computeFinishTime(horse, 2200)).toBeGreaterThan(
      computeFinishTime(horse, 1200),
    );
    vi.restoreAllMocks();
  });
});
