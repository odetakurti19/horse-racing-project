...
describe("raceStore — mutations", () => {
  ...
  it("COMPLETE_ROUND appends to completedResults", () => {
    const store = makeStore();
    store.commit("race/SET_SCHEDULE", buildSchedule(generateHorses()));
    const results = [stubResult()];
    store.commit("race/COMPLETE_ROUND", results);
    expect(store.state.race.completedResults).toHaveLength(1);
    expect(store.state.race.completedResults[0]).toEqual(results);
  });
  ...
  it("RESET restores the initial state", () => {
    ...
    expect(store.state.race.completedResults).toEqual([]);
  });
});
...
describe("raceStore — actions", () => {
  ...
  it("completeRound stores results", async () => {
    const store = makeStore();
    store.commit("race/SET_SCHEDULE", buildSchedule(generateHorses()));
    const results = [stubResult()];
